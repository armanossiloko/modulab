...
**Control Center:** everything under [`control-center/`](control-center/) — Angular UI in `ui/`, defaults in `defaults/`, generated runtime files in `wwwroot/` (not in git), API in `src-backend/Modulab.ControlCenter/`. Open **`Modulab.slnx`**.
...
UI is generated by `bash scripts/setup.sh` / `ensure-wwwroot.sh` (+ `publish-ui.sh` or Docker build). Dev: `cd control-center/ui && npm start` (proxies `/api` to :8888).
